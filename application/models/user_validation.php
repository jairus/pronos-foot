...
class user_validation extends CI_Model {
	...
	public function validate($class, $function="%", $die=true){
		...
		if($_SESSION['user']['email']=="admin"){ //return true to all if admin
			...
			$sql = "select * from `user_permissions` where `class_name`='".db_escape($class)."' and `function`='-".db_escape($function)."' and `deleted`<>1";
			...
			$sql = "select * from `user_permissions` where `class_name`='".db_escape($class)."' and `function`='%' and `deleted`<>1";
			if(trim($usgtxt)){
				$sql .= " and ( ".$usgtxt." ) ";
			}
			else{
			...
			$sql = "select * from `user_permissions` where `class_name`='".db_escape($class)."' and `deleted`<>1";
			if(trim($usgtxt)){
				$sql .= " and ( ".$usgtxt." ) ";
			}
			else{
