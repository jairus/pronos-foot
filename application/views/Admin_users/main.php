				...
				<div class="form-group fg-line">
					...
					if(isset($filters)&&is_array($filters)){
						...
						<select class="selectpicker" multiple name="filters[]">
							<option value="name" <?php if(in_array("name", $filters)){ echo "selected"; } ?>>Name</option>	<option value="email" <?php if(in_array("email", $filters)){ echo "selected"; } ?>>Login Name</option>	
						</select>
						<?php
					}
					else{
						?>
						<select class="selectpicker" multiple name="filters[]">
							<option value="name">Name</option>	<option value="email">Login Name</option>	
						</select>
						<?php
					}
					?>
	...
	<div class="table-responsive">
		...
		<table class="table table-bordered">
			<thead>
				<tr>
					<th>#</th>
					<th class="sortby" data-sortby="name" >Name <?php
	if($sortby=="name"){
		if($sort=="asc"){
...
?></th><th class="sortby" data-sortby="email" >Login Name <?php
	if($sortby=="email"){
		if($sort=="asc"){
			echo '&nbsp;<i class="zmdi zmdi-caret-up"></i>';
		}
...
?></th><th class="sortby" data-sortby="dateadded" >Dateadded <?php
	if($sortby=="dateadded"){
		if($sort=="asc"){
			echo '&nbsp;<i class="zmdi zmdi-caret-up"></i>';
		}
				...
				if($t){
					for($i=0; $i<$t; $i++){
						...
						?>
							...
							<td><?php echo $records[$i]['name'];?></td><td><?php echo $records[$i]['email'];?></td><td><?php echo date("M d, Y H:i:s", strtotime($records[$i]['dateadded']));?></td>
							<td class="text-left">
							<?php
							...
							else if($records[$i]['email']=="admin" && $_SESSION['user']['email']=='admin'){
								?>
								<a href="<?php echo site_url(); ?><?php echo $controller; ?>/edit/<?php echo $records[$i]['id']?>" class="btn btn-icon command-edit waves-effect waves-circle bgm-green"><span class="zmdi zmdi-edit"></span></a>
								<?php
